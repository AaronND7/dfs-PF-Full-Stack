// Servidor con puerto correcto para Google OAuth
console.log('Iniciando servidor con configuración correcta para Google...');
...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/') {
    ...
    res.end(JSON.stringify({ status: 'API running 🎵 - Puerto 3000' }));
    return;
  }
  ...
  if (req.url === '/auth/google') {
    ...
    const redirectUri = 'http://localhost:3000/auth/google/callback'; // Puerto 3000 como Google espera
...
const PORT = 3000; // Puerto 3000 para coincidir con Google
server.listen(PORT, () => {
  console.log(`🚀 SERVIDOR INICIADO en http://localhost:${PORT}`);
  console.log('✅ Ruta /auth/google disponible');
  console.log('✅ Configuración compatible con Google OAuth');
...
server.on('error', (err) => {
  ...
  if (err.code === 'EADDRINUSE') {
    console.log('💡 El puerto 3000 está ocupado. Detén otros procesos Node.js');
  }
});
